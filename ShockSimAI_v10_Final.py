...
st.title("🧠 Shock Sim AI — Adaptive Resilience Intelligence v4.1")
...
def portfolio_capital_curve(results):
    """Summarize the best containment reachable at each observed investment level."""
    if not results:
        return pd.DataFrame()
    rows = []
    for r in results:
        rows.append({
            "Investment ($k)": round(float(r["cost"]), 2),
            "Containment (%)": float(r["containment"]),
            "Avoided Damage": float(r["damage_avoided"]),
        })
    df = pd.DataFrame(rows)
    df = df.sort_values(["Investment ($k)", "Containment (%)"], ascending=[True, False])
    return df.groupby("Investment ($k)", as_index=False).first()
...
def model_health_report(graph, memory, validation_df):
    ...
    checks = [
        ...
        ("Validation available", validation_df is not None and not validation_df.empty),
    ]
...
st.caption(
    "Shock Sim AI v10.0 — Resilience Operating System | "
